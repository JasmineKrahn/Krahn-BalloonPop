...
bool MyObject::CollidedWith(MyObject * b) const
{
	...
	return ((x + image->Width) >= b->x && (b->x + b->image->Width) >= x && (y + image->Height) >= b->y && (b->y + b->image->Height) >= y);
}
...
void MyObject::setScores(MyObject* b)
{
	...
	// ballon vs. bomb
	if (b->typeOfObject() == 0 && this->typeOfObject() > 0) //change so that the surprise would be ignored
	{
	...
	// bomb vs balloon
	else if (this->typeOfObject() == 0 && b->typeOfObject() > 0) //change so that the surprise would be ignored
	{
		// add the points for the balloon
		value += b->popScore();
